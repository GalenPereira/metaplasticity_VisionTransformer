...
class BinarizeBatchNorm2d(nn.Module):
    def __init__(self, num_features):
        super(BinarizeBatchNorm2d, self).__init__()
...
class BinarizedLayerNorm(nn.Module):
    def __init__(self, normalized_shape, eps=1e-5, elementwise_affine=True):
        super(BinarizedLayerNorm, self).__init__()
...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, img_size, patch_size, in_chans=3, embed_dim=768):
        ...
        print("N-pateches====",self.n_patches)
...
class BinarizeMLP(nn.Module):
    ...
    def __init__(self, in_features, hidden_features, out_features, p=0.):
        super().__init__()
        self.bn1 = BinarizeLinear(in_features, hidden_features)
        self.act = nn.GELU()
        self.bn2 = BinarizeLinear(hidden_features, out_features)
    ...
    def forward(self, x):
        ...
        """
        x = self.bn1(x) # (n_samples, n_patches + 1, hidden_features)
        x = self.act(x)  # (n_samples, n_patches + 1, hidden_features)
        x = self.drop(x)  # (n_samples, n_patches + 1, hidden_features)
        x = self.bn2(x)  # (n_samples, n_patches + 1, out_features)
...
class BinarizeBlock(nn.Module):
    ...
    def __init__(self, dim, n_heads, mlp_ratio=4.0, qkv_bias=True, p=0., attn_p=0.):
        ...
        self.norm2 = BinarizedLayerNorm(dim, eps=1e-6)
        hidden_features = int(dim * mlp_ratio)
        self.mlp = BinarizeMLP(
                in_features=dim,
                hidden_features=hidden_features,
                out_features=dim,
        )
...
class BinarizeVisionTransformer(nn.Module):
    """
	Attributes
	----------
